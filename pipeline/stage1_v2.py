...
def score_window(row: dict, baseline: CohortBaseline, calibration: dict, config: dict) -> dict:
    ...
    evidence_score = 0.0
    if evidence.get("period_mismatch") is True:
        evidence_score += weights.get("period_mismatch", 1.0)
    if evidence.get("unexplained_changepoint_count", 0) >= 1:
        evidence_score += weights.get("unexplained_changepoint", 0.8)
    if float(evidence.get("cross_job_sync_index", 0.0) or 0.0) >= float(config.get("tau_sync", 0.8)):
        evidence_score += weights.get("cross_job_sync", 1.0)
    if evidence.get("progress_log_missing") and row.get("declared_job_family") == "training":
        evidence_score += weights.get("progress_log_missing", 0.25)
